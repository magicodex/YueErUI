...
/**
 * @class
 * @classdesc 表示 script 标签
 * @param {object} [opts]
 */
function HtmlScripts(opts) {
 ...
 * @param {object} [opts]
 */
HtmlScripts.prototype.addScript = function (url, opts) {
  ...
  this.addScriptFmt('<script src="{0}"></script>', url);
};

/**
 * @description 字符串格式化后添加脚本
 * @param {string} format 脚本标签格式
 * @param {string} url 脚本 URL
 */
HtmlScripts.prototype.addScriptFmt = function (format, url) {
  ...
  if (!utils.isString(url)) {
    throw new Error('argument#1 "url" required string');
  }
 ...
 * @param {Writer} writer 
 */
HtmlScripts.prototype.renderToString = function (writer) {
